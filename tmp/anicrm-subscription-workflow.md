...
**Важно:** Все цены указаны с НДС 20%. Оплата в рублях через российский платежный шлюз.
...
sequenceDiagram
    ...
    B->>B: Запланировать напоминания (День 12)
    
    B-->>F: 201 Created (user, subscription)
    F->>U: Немедленный доступ в систему с туром
    
    Note over U,E: Trial активен 14 дней<br/>План: VIP (все функции)<br/>Email подтверждение опционально
...
6. Видит checkout страницу с расчетом (49 900₽ с НДС)
